...
module.exports = async function(deployer,_network,accounts) {
    ...
    const depositBalanceToExchange = async (token, trader) => {
    ...
    await Promise.all(
    [dai, bat, rep, zrx].map(
        token => depositBalanceToExchange(token, trader1) 
    )
    );
    await Promise.all(
    [dai, bat, rep, zrx].map(
        token => depositBalanceToExchange(token, trader2) 
    )
    );
    await Promise.all(
        [dai, bat, rep, zrx].map(
            token => depositBalanceToExchange(token, trader3) 
        )
    );

    await Promise.all(
        [dai, bat, rep, zrx].map(
            token => depositBalanceToExchange(token, trader4) 
        )
    );
